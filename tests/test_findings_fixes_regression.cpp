/// BAMSI Findings & Fixes Regression Tests — Execution Plan §4.3.3
...
std::string BuildIndex(const std::string& name, const std::string& extra = "") {
    std::string bsi = "/tmp/bamsi_ff_" + name + ".bsi";
...
TEST(FindingsFixes, F2_BaseTierQueries) {
    ...
    for (const auto& pat : patterns) {
        auto r = RunBamsi("count --index " + bsi + " --pattern " + pat);
...
TEST(FindingsFixes, F4_BlockSizeIndependence) {
    ...
    auto r1 = RunBamsi("count --index " + bsi1 + " --pattern ACGT");
    auto r2 = RunBamsi("count --index " + bsi2 + " --pattern ACGT");
...
TEST(FindingsFixes, S3_ParallelSABitIdentity) {
    ...
    for (const auto& pat : patterns) {
        auto r1 = RunBamsi("count --index " + bsi_seq + " --pattern " + pat);
        auto r2 = RunBamsi("count --index " + bsi_par + " --pattern " + pat);
...
TEST(FindingsFixes, S6_StrandCompleteCounting) {
    ...
    auto r_single = RunBamsi("count --index " + bsi + " --pattern ACG --strand single");
    auto r_complete = RunBamsi("count --index " + bsi + " --pattern ACG --strand complete");
...
TEST(FindingsFixes, C1_OutputOrderingModes) {
    ...
    auto r_unsorted = RunBamsi("locate --index " + bsi + " --pattern ACGT --json");
    auto r_sorted = RunBamsi("locate --index " + bsi + " --pattern ACGT --sort-output --json");
...
TEST(LossyMode, ReconstructRefusesWithoutFlag) {
    ...
    auto r = RunBamsi("reconstruct --index " + bsi +
                       " --output /tmp/bamsi_ff_lossy_refuse.bam");
...
TEST(LossyMode, ReconstructSucceedsWithFlag) {
    ...
    auto r = RunBamsi("reconstruct --index " + bsi +
                       " --output /tmp/bamsi_ff_lossy_ok.bam --allow-lossy");
...
TEST(Provenance, VerifyDetectsTampering) {
    ...
    auto r1 = RunBamsi("verify --index " + bsi);
    EXPECT_EQ(r1.exit_code, 0);

    // Tamper with the file (flip a byte near the end)
    std::string tampered = "/tmp/bamsi_ff_tampered.bsi";
    {
    ...
    auto r2 = RunBamsi("verify --index " + tampered);
...
TEST(Provenance, NoNetworkRequired) {
    // bamsi count should succeed without network access
...
TEST(CodecAblation, EntropyKVariants) {
    ...
    auto r1 = RunBamsi("count --index " + bsi_k2 + " --pattern ACGT");
    auto r2 = RunBamsi("count --index " + bsi_k4 + " --pattern ACGT");
    EXPECT_EQ(r1.exit_code, 0);
    EXPECT_EQ(r2.exit_code, 0);
...
TEST(CodecAblation, SampleStepVariants) {
    ...
    auto r1 = RunBamsi("count --index " + bsi_s32 + " --pattern ACGT");
    auto r2 = RunBamsi("count --index " + bsi_s128 + " --pattern ACGT");
    EXPECT_EQ(r1.output, r2.output)
        << "Different sample_step should not affect count results";

    // Locate must return same positions
    auto l1 = RunBamsi("locate --index " + bsi_s32 + " --pattern ACGT --sort-output --json");
    auto l2 = RunBamsi("locate --index " + bsi_s128 + " --pattern ACGT --sort-output --json");
