/// BAMSIX TIER 2 Integration Tests — Contract §S4 / Execution Plan §4.3
/// Tests: Error handling sweep, codec correctness, lossy mode, CLI flags.
/// Build: linked against bamsix-core + gtest
...
const std::string kTmpDir = "/tmp/bamsix_tier2_";

std::string TmpFile(const std::string& name) {
    return kTmpDir + name;
}

// Helper: run bamsix CLI and return exit code + stdout
struct CliResult {
    int exit_code;
    std::string output;
};
...
CliResult RunBamsix(const std::string& args) {
    std::string cmd = "./build/bamsix " + args + " 2>/dev/null";
...
TEST_F(ErrorHandlingTest, EmptyPatternCountReturnsError) {
    auto r = RunBamsix("count --index " + bsi_path_ + " --pattern \"\"");
...
TEST_F(ErrorHandlingTest, NonexistentIndexReturnsError) {
    auto r = RunBamsix("count --index /nonexistent.bsi --pattern ACGT");
    EXPECT_NE(r.exit_code, 0);
}
...
TEST_F(ErrorHandlingTest, LossyReconstructWithoutFlagFails) {
    ...
    ASSERT_EQ(system(cmd.c_str()), 0);
    auto r = RunBamsix("reconstruct --index " + lossy + " --output /tmp/nope.bam");
    EXPECT_NE(r.exit_code, 0);
}
...
TEST(CAbiTest, NullPointerReturnsError) {
    EXPECT_EQ(bamsix_open(nullptr, nullptr), BAMSIX_STATUS_INVALID_ARGUMENT);
    EXPECT_EQ(bamsix_verify(nullptr, nullptr), BAMSIX_STATUS_INVALID_ARGUMENT);
    EXPECT_EQ(bamsix_global_count(nullptr, nullptr, 0, nullptr),
...
TEST(CAbiTest, VersionReturnsNonNull) {
    EXPECT_NE(bamsix_version(), nullptr);
...
TEST(CAbiTest, OpenAndQueryRoundTrip) {
    ...
    uint64_t count = 0;
    EXPECT_EQ(bamsix_global_count(idx, pat, 4, &count), BAMSIX_STATUS_OK);
    EXPECT_GT(count, 0u);

    // GlobalExists
    int exists = 0;
    EXPECT_EQ(bamsix_global_exists(idx, pat, 4, 1, &exists), BAMSIX_STATUS_OK);
    EXPECT_EQ(exists, 1);

    // Locate
    bamsix_locate_result_t results[100];
    size_t n_results = 0;
    EXPECT_EQ(bamsix_locate(idx, pat, 4, results, 100, &n_results), BAMSIX_STATUS_OK);
    EXPECT_EQ(n_results, count);

    // Info getters
    uint64_t n_reads = 0;
    EXPECT_EQ(bamsix_get_n_reads(idx, &n_reads), BAMSIX_STATUS_OK);
    EXPECT_EQ(n_reads, 10u);

    uint64_t s_len = 0;
    EXPECT_EQ(bamsix_get_s_length(idx, &s_len), BAMSIX_STATUS_OK);
    EXPECT_EQ(s_len, 149u);

    uint32_t n_windows = 0;
    EXPECT_EQ(bamsix_get_n_windows(idx, &n_windows), BAMSIX_STATUS_OK);
    EXPECT_GE(n_windows, 1u);

    int lossless = 0;
    EXPECT_EQ(bamsix_is_lossless(idx, &lossless), BAMSIX_STATUS_OK);
    EXPECT_EQ(lossless, 1);

    uint32_t chrom_count = 0;
    EXPECT_EQ(bamsix_get_chrom_count(idx, &chrom_count), BAMSIX_STATUS_OK);
    EXPECT_GE(chrom_count, 1u);

    char chrom_buf[256];
    size_t chrom_len = 0;
    EXPECT_EQ(bamsix_get_chrom_name(idx, 0, chrom_buf, 256, &chrom_len), BAMSIX_STATUS_OK);
    EXPECT_GT(chrom_len, 0u);

    // Regional query
    uint64_t reg_count = 0;
    EXPECT_EQ(bamsix_regional_count(idx, pat, 4, chrom_buf, 0, UINT64_MAX, &reg_count),
              BAMSIX_STATUS_OK);

    int reg_exists = 0;
    EXPECT_EQ(bamsix_regional_exists(idx, pat, 4, chrom_buf, 0, UINT64_MAX, 1, &reg_exists),
              BAMSIX_STATUS_OK);

    // Verify
    int valid = 0;
    EXPECT_EQ(bamsix_verify(bsi.c_str(), &valid), BAMSIX_STATUS_OK);
    EXPECT_EQ(valid, 1);
...
TEST(CodecTest, RoundTripLossless) {
    ...
    std::string cmd1 = "./build/bamsix build --input " + kTestBam +
                       " --output " + bsi1 + " 2>/dev/null";
    std::string cmd2 = "./build/bamsix build --input " + kTestBam +
                       " --output " + bsi2 + " 2>/dev/null";
    ASSERT_EQ(system(cmd1.c_str()), 0);
    ASSERT_EQ(system(cmd2.c_str()), 0);

    // Both should verify
    auto r1 = RunBamsix("verify --index " + bsi1);
    auto r2 = RunBamsix("verify --index " + bsi2);
    EXPECT_EQ(r1.exit_code, 0);
    EXPECT_EQ(r2.exit_code, 0);

    // Same count for any pattern
    auto c1 = RunBamsix("count --index " + bsi1 + " --pattern ACGT");
    auto c2 = RunBamsix("count --index " + bsi2 + " --pattern ACGT");
...
TEST(CliTest, InfoJsonHasAllFields) {
    ...
    auto r = RunBamsix("info --index " + bsi + " --json");
    EXPECT_EQ(r.exit_code, 0);
...
TEST(CliTest, LocateSortOutput) {
    ...
    auto r = RunBamsix("locate --index " + bsi +
...
TEST(CliTest, RegionFlagParsing) {
    ...
    auto r = RunBamsix("region-count --index " + bsi +
...
TEST(CliTest, StrandFlagOverride) {
    ...
    auto r_complete = RunBamsix("count --index " + bsi +
                                " --pattern ACG --strand complete");
    auto r_single = RunBamsix("count --index " + bsi +
...
TEST(CliTest, VerifyJson) {
    ...
    auto r = RunBamsix("verify --index " + bsi + " --json");
    EXPECT_EQ(r.exit_code, 0);
