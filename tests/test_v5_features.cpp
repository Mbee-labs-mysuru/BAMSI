/// BAMSIX V5 Feature Tests — ISA samples, SARange wavelet tree, Reverse FM-index
...
std::string BuildIndex(const std::string& name, const std::string& extra = "") {
    std::string bsi = "/tmp/bamsix_v5_" + name + ".bsi";
...
TEST(V5Features, ISASamplesQueryConsistency) {
    ...
    for (const auto& pat : patterns) {
        auto r1 = RunBamsix("count --index " + bsi_no_isa + " --pattern " + pat);
        auto r2 = RunBamsix("count --index " + bsi_isa + " --pattern " + pat);
        EXPECT_EQ(r1.output, r2.output) << "ISA samples changed count for: " << pat;

        // Locate positions should also match
        auto l1 = RunBamsix("locate --index " + bsi_no_isa + " --pattern " + pat + " --sort-output --json");
        auto l2 = RunBamsix("locate --index " + bsi_isa + " --pattern " + pat + " --sort-output --json");
...
TEST(V5Features, SARangeQueryConsistency) {
    ...
    std::vector<std::string> patterns = {"A", "AC", "ACG", "ACGT"};
    for (const auto& pat : patterns) {
        auto r1 = RunBamsix("count --index " + bsi_base + " --pattern " + pat);
        auto r2 = RunBamsix("count --index " + bsi_enhanced + " --pattern " + pat);
...
TEST(SARangeUnit, SmallArrayRangeCount) {
    ...
    wt.Build(data, 10, 1);  // quantize_step=1 for exact unit-level counting
...
TEST(V5Features, FullV5BuildAndQuery) {
    ...
    // Query should work
    auto r = RunBamsix("count --index " + bsi + " --pattern ACGT");
    EXPECT_EQ(r.exit_code, 0);
    int count = std::stoi(r.output);
    EXPECT_GT(count, 0);

    // Verify should pass
    auto rv = RunBamsix("verify --index " + bsi);
    EXPECT_EQ(rv.exit_code, 0);

    // Info should show both features
    auto ri = RunBamsix("info --index " + bsi);
...
TEST(V5Features, FullV5Reconstruct) {
    ...
    auto r = RunBamsix("reconstruct --index " + bsi +
                       " --output /tmp/bamsix_v5_recon.bam");
    EXPECT_EQ(r.exit_code, 0);
}
