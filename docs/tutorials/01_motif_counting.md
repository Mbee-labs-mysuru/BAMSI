...
- BAMSIX built and available in `$PATH` or `build/` directory
- A BAM file (we use the synthetic test data)

## Step 1: Build the Index

```bash
bamsix build data/test/synthetic_10reads.bam -o motif_demo.bsi
```
...
bamsix count --index motif_demo.bsi --pattern ACGT
# Output: 13

# Count a specific sequence
bamsix count --index motif_demo.bsi --pattern CCGGTTAA
# Output: 5 (3 forward + 2 reverse complement)

# Check if a motif exists
bamsix exists --index motif_demo.bsi --pattern AACCGGTT
# Output: true
```
...
for motif in ACGT CCGG TTAA AACCGGTT GGTTAACC; do
    count=$(bamsix count --index motif_demo.bsi --pattern $motif)
    echo "$motif: $count"
done
```
...
bamsix count --index motif_demo.bsi --pattern ACGT --json
# Output: {"count":13}

# Pipe to jq for processing
bamsix count --index motif_demo.bsi --pattern ACGT --json | jq '.count'
```
...
bamsix locate --index motif_demo.bsi --pattern ACGT --sorted
```
