...
```bash
bamsix build data/test/synthetic_10k.bam -o region_demo.bsi
```

## Step 2: Inspect the Index

```bash
bamsix info --index region_demo.bsi
```

Output:
```
BAMSIX Index: region_demo.bsi
  Format version:   6
  |S|:              1011701
  Reads:            10000
  Windows:          11
...
bamsix count --index region_demo.bsi --pattern ACGT
# Output: some large number

# Regional count: how many times in chr1:1000-50000?
bamsix region-count --index region_demo.bsi --pattern ACGT \
...
bamsix region-exists --index region_demo.bsi --pattern ACGTACGT \
    --chrom chr3 --start 1 --end 1000000

# With threshold: at least 5 occurrences?
bamsix region-exists --index region_demo.bsi --pattern ACGT \
...
for chrom in chr1 chr2 chr3 chr4 chr5; do
    count=$(bamsix region-count --index region_demo.bsi --pattern ACGT \
