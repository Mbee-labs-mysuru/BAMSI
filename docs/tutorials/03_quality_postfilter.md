...
Two-phase workflow: first identify candidate patterns via BAMSI, then post-filter using quality scores.
...
1. **Phase 1**: Use BAMSI to efficiently count/locate candidate patterns
...
```bash
bamsi build data/test/synthetic_10reads.bam -o quality_demo.bsi
```
...
bamsi locate --index quality_demo.bsi --pattern ACGT --json
```
...
bamsi reconstruct --index quality_demo.bsi --read-id 0
```
...
count=$(bamsi count --index $INDEX --pattern $PATTERN)
echo "Phase 1: $count occurrences of $PATTERN found"

# Phase 2: Check if index is lossless
lossless=$(bamsi info --index $INDEX --json 2>/dev/null | grep -o '"is_lossless": *[a-z]*' | grep -o '[a-z]*$')
...
4. **The two-phase approach** is efficient: BAMSI narrows candidates to a small subset, then quality filtering operates on that subset
