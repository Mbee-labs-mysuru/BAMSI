...
void ReverseFMIndex::Build(const std::vector<int64_t>& forward_sa,
    ...
    // ─── Step 2: Derive reverse SA from forward SA (Architecture §4.6.7) ─────
    // For the reverse text S_R$, we need SA_R such that S_R$[SA_R[i]..] are
    // in sorted order. We derive this from forward SA without re-running SA-IS.
    //
    // Key insight: ISA[SA[j]] = j. We invert forward SA to get ISA, then
    // for each text position q in S_R (where S_R[q] = S[n-1-q]),
    // the reverse suffix starting at q corresponds to the forward suffix
    // starting at (n-1-q), reversed.
    std::vector<int64_t> isa(sa_len);
    for (int64_t j = 0; j < sa_len; ++j) {
        isa[forward_sa[j]] = j;
    }

    // Build SA_R by sorting positions [0..n] of S_R$ lexicographically.
    // We use the relationship between forward and reverse suffixes.
    std::vector<int64_t> sa_r(sa_len);

    // Create pairs (reverse_pos, fwd_row) and sort by S_R suffix order
    // Simple O(n) approach: for each position p in [0..n], SA_R maps
    // ranks of reversed suffixes.
    std::vector<int64_t> positions(sa_len);
    for (int64_t i = 0; i <= n; ++i) positions[i] = i;

    // Sort positions by S_R$ suffix. Position n is the sentinel (smallest).
    std::sort(positions.begin(), positions.begin() + sa_len,
              [&](int64_t a, int64_t b) {
        if (a == n) return true;   // sentinel is smallest
        if (b == n) return false;
        // Compare S_R[a..] vs S_R[b..]
        for (int64_t k = 0; a + k < n && b + k < n; ++k) {
            if (S_R[a + k] != S_R[b + k]) return S_R[a + k] < S_R[b + k];
        }
        // Shorter suffix (reaching $) is smaller
        return (n - a) < (n - b);
    });
