...
void BuildIndex(const std::vector<std::string>& bam_paths,
    ...
    auto seq_encoded = EncodeSeqStream(sais.BWT, config.entropy_order_k);
    std::cerr << "[build]   S_seq payload: " << seq_encoded.payload.size() << " bytes"
              << " (" << elapsed() << " ms)\n";

    // ─── Stage 5b: FM-index construction ─────────────────────────────────────
    ...
    if (config.enable_sarange) {
        ...
        sarange.Build(fm.SASamples(), bundle.S.size());
    ...
    seal_input.seq = seq_encoded;
    seal_input.qual = qual_encoded;
    seal_input.meta = meta_encoded;
    seal_input.map = map_encoded;
    seal_input.fm = fm;
    seal_input.bv = bv;
    seal_input.windows = windows;
    seal_input.reads = ordering.reads;

    // V5 ENHANCED features
    if (fm.HasISASamples()) {
        seal_input.isa_samples = fm.ISASamples();
        seal_input.isa_step = fm.ISAStep();
    }
    if (sarange.IsBuilt()) {
        seal_input.sarange = sarange;
    }
