...
WindowTable BuildWindows(const std::vector<OrderedRead>& reads,
    ...
    for (const auto& [chrom_id, range] : chrom_ranges) {
        ...
        while (idx <= range.last_idx) {
            ...
            // Contract §3.5.2: warn if T < |r_i| (oversized read)
            if (start_read.seq.size() > T) {
                std::cerr << "WARNING: read " << start_read.read_id
                          << " has length " << start_read.seq.size()
                          << " > window_size T=" << T
                          << ". Window may contain only this read.\n";
            }
