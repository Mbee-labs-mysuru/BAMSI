...
public:
    ...
    /// H7 fix (I15): canonical name enforces closed-interval convention,
    /// distinct from half-open BWT rank in OccTable.
    uint64_t RankClosed(uint64_t pos) const;

    /// Alias for RankClosed — backward compatibility.
    uint64_t Rank1(uint64_t pos) const { return RankClosed(pos); }
...
/// Bitvector pair for the BAMSIX build: B_read and B_window.
struct BitVectors {
