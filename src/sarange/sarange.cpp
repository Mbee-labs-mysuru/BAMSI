...
void SARange::Build(const std::vector<uint64_t>& sa_samples, uint64_t max_val,
                    uint64_t quantize_step) {
    quantize_step_ = (quantize_step > 0) ? quantize_step : 1;
    n_ = sa_samples.size();
    max_val_ = max_val / quantize_step_;

    if (n_ == 0 || max_val_ == 0) return;

    // Number of levels = ceil(log2(max_val_ + 1))
    // With quantize_step = s: max_val_ = |S|/s, so depth = O(log(|S|/s))
    n_levels_ = 0;
    ...
    // Working arrays: quantize values before building the tree
    std::vector<uint64_t> current(sa_samples.size());
    for (size_t i = 0; i < sa_samples.size(); ++i) {
        current[i] = sa_samples[i] / quantize_step_;
    }
...
uint64_t SARange::RangeCount(uint64_t lo, uint64_t hi,
    ...
    // Quantize query bounds (sound: floor division gives superset for pruning)
    // Architecture §5.3: range_count operates on quantized value space
    val_lo = val_lo / quantize_step_;
    val_hi = val_hi / quantize_step_;
...
std::vector<uint8_t> SARange::Serialize() const {
    ...
    std::memcpy(out.data() + off, &quantize_step_, sizeof(quantize_step_)); off += sizeof(quantize_step_);
    std::memcpy(out.data() + off, &n_levels_, sizeof(n_levels_));
...
void SARange::Deserialize(const uint8_t* data, size_t len) {
    if (len < sizeof(uint64_t) * 3 + sizeof(uint32_t)) return;

    size_t off = 0;
    std::memcpy(&n_, data + off, sizeof(n_)); off += sizeof(n_);
    std::memcpy(&max_val_, data + off, sizeof(max_val_)); off += sizeof(max_val_);
    std::memcpy(&quantize_step_, data + off, sizeof(quantize_step_)); off += sizeof(quantize_step_);
