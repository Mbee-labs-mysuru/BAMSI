...
///   Complexity: O(log(max_val / quantize_step)) = O(log(|S|/s)) per Architecture §5.3
///
...
public:
    ...
    /// quantize_step divides values to reduce tree depth: O(log(max_val/quantize_step)).
    /// When quantize_step = s (SA sample step), depth = O(log(|S|/s)) per Architecture §5.3.
    void Build(const std::vector<uint64_t>& sa_samples, uint64_t max_val,
               uint64_t quantize_step = 1);

    /// Count elements in sa_samples[lo..hi) that fall within [val_lo, val_hi].
    /// Complexity: O(log(max_val / quantize_step)) = O(log(|S|/s))
    /// Per Architecture §5.3: range_count(lo, hi, l_j, r_j)
    /// Bounds are quantized internally; may return a superset count (sound for pruning).
...
private:
    ...
    uint64_t max_val_ = 0;       // quantized universe size (max_val / quantize_step)
    uint64_t quantize_step_ = 1; // value quantization step (= s for O(log(|S|/s)))
    uint32_t n_levels_ = 0;      // number of levels = ceil(log2(max_val / quantize_step))
