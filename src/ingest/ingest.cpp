    ...
    for (uint32_t file_id = 0; file_id < bam_paths.size(); ++file_id) {
        ...
        while (sam_read1(fp, hdr, rec) >= 0) {
            ...
            if (is_mapped && is_primary && is_not_supp && has_valid_pos) {
                ...
                // QNAME (read name) — required for lossless reconstruction
                read.qname = std::string(bam_get_qname(rec));

                // MAPQ (mapping quality)
                read.mapq = rec->core.qual;
                ...
                // BAM aux tags — raw bytes (Contract §2.2: S_meta = CIGAR + FLAG + tags)
                uint8_t* aux_ptr = bam_get_aux(rec);
                int aux_len = bam_get_l_aux(rec);
                if (aux_len > 0 && aux_ptr) {
                    read.aux_data.assign(aux_ptr, aux_ptr + aux_len);
                }
