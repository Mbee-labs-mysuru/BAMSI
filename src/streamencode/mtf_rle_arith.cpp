...
std::vector<uint8_t> ArithEncode(const std::vector<uint8_t>& input) {
    ...
    for (auto sym : input) {
        ...
        hi = lo + (uint32_t)((uint64_t)range * model.cum[sym + 1] / model.total) - 1;
        lo = lo + (uint32_t)((uint64_t)range * model.cum[sym] / model.total);
        ...
        while (true) {
            if ((lo ^ hi) < ARITH_TOP) {
                ...
                lo <<= 8;
                hi = (hi << 8) | 0xFF;
            } else if (lo >= ARITH_BOT && hi < 0xFF000000u) {
                // Underflow: shift out middle byte
                lo = (lo << 8) & 0xFFFFFF00u;
                hi = (hi << 8) | 0xFF;
                // Adjust: keep high byte, shift
                break;
            } else {
                break;
            }
...
std::vector<uint8_t> ArithDecode(const std::vector<uint8_t>& encoded, uint64_t expected_len) {
    ...
    for (uint64_t n = 0; n < expected_len; ++n) {
        uint32_t range = hi - lo + 1;
        uint32_t scaled = (uint32_t)(((uint64_t)(code - lo) * model.total) / range);

        // Find symbol
        int sym = 0;
        for (int s = 0; s < nsym; ++s) {
            if (model.cum[s + 1] > scaled) { sym = s; break; }
        }
        ...
        while (true) {
            if ((lo ^ hi) < ARITH_TOP) {
                lo <<= 8;
                hi = (hi << 8) | 0xFF;
                code = (code << 8) | (pos < encoded.size() ? encoded[pos++] : 0);
            } else if (lo >= ARITH_BOT && hi < 0xFF000000u) {
                lo = (lo << 8) & 0xFFFFFF00u;
                hi = (hi << 8) | 0xFF;
                code = (code << 8) | (pos < encoded.size() ? encoded[pos++] : 0);
